...
/*
 * This mongo query is specific to /api/order/:type.
 */
const mongoQuery = {
...
/*
 * Only two API endpoints talks to mongodb, /api/kanji/:kanji and /api/order/:type.
 */
const operations = {
  findKanji: 'findKanji',
  findOrder: 'findOrder',
...
/*
 * Removes ID generated by Mongo because its not a particularly interesting metadata.
 */
...
/*
 * This util function is only used when grabbing resource for /api/order/:type.
 * Creates a flat result of orders from results.
 * results is an array of kanji objects (see specification under schema/kanji-schema.json).
 * Returned object looks like: {"亜":8,"娃":9,...}
 */
const createResultObject = (results, key) => {
  ...
  // forEach is async... so we use a normal for loop here
...
const checkMongoForValue = (key, operation) => {
  return new Promise((resolve) => {
    ...
    // If the user is asking for the order API, then it needs to be one of the specified orders
